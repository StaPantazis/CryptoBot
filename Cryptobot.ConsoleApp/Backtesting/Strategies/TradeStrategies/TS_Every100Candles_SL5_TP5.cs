...
public class TS_Every100Candles_SL5_TP5 : TradeStrategyBase
{
    ...
    protected override bool ShouldShort<T>(CacheService cacheManager, List<T> candles, int currentCandleIndex, CandleInterval _) => currentCandleIndex % 400 == 0;

    protected override bool ShouldLong<T>(CacheService cacheManager, List<T> candles, int currentCandleIndex, CandleInterval _) => currentCandleIndex % 400 != 0 && currentCandleIndex % 100 == 0;
}
