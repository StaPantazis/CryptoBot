...
{
    public static StrategyBundle<TBudgetStrategy>[] Sandbox<TTradeStrategy, TBudgetStrategy>(CacheService cache)
    ...
    public static double?[] ArrayLoops(params double?[]? nums) => nums is null || !nums.Any() ? [null] : nums;
    public static double[] Low1To6Percent() => ArrayLoops(0.94, 0.95, 0.96, 0.97, 0.98, 0.99).Cast<double>().ToArray();
    public static double[] High1To6Percent() => ArrayLoops(1.01, 1.02, 1.03, 1.04, 1.05, 1.06).Cast<double>().ToArray();
    public static TrendConfiguration[] AllTrendStrategies()
