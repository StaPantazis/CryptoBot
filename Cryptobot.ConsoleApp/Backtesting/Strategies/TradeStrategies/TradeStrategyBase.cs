...
public abstract class TradeStrategyBase : StrategyBase
{
    ...
    public virtual bool ShouldOpenTrade<T>(CacheService cacheManager, List<T> candles, int currentCandleIndex, out PositionSide? position) where T : Candle
    {
        if (Spot.Budget < 10)
        {
            position = null;
    ...
    protected abstract bool ShouldShort<T>(CacheService cacheManager, List<T> candles, int currentCandleIndex) where T : Candle;
    protected abstract bool ShouldLong<T>(CacheService cacheManager, List<T> candles, int currentCandleIndex) where T : Candle;
}
