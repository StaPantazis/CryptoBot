...
public class TS_LongWhenHigherThanMovingAverage_SL5_TP5 : TradeStrategy
{
    public override string Name { get; } = "Trade when Moving Average higher than last 300 MA | SL -5% | TP +5%";
    public override string NameOf { get; } = nameof(TS_LongWhenHigherThanMovingAverage_SL5_TP5);
    ...
    protected override bool ShouldLong<T>(List<T> candles, int currentCandleIndex)
    {
        ...
        return last300.All(x => x < candle.ClosePrice);
    }
}
