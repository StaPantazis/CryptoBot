...
public class TS_Simple_120_days_SMA_Mix : TradeStrategyBase
{
    public override string Name { get; protected set; } = "SMA Min_Long_Short";
    ...
    protected override bool ShouldLong<T>(CacheService cacheManager, List<T> candles, int currentCandleIndex)
    {
        ...
        var previousCandle = candles[currentCandleIndex - 1];
        var candle = candles[currentCandleIndex];
        return candle.ClosePrice > cacheManager.MacroTrendCache[candle.DayTicks].MA120d
            && previousCandle.ClosePrice < cacheManager.MacroTrendCache[candle.DayTicks].MA120d;
    }
