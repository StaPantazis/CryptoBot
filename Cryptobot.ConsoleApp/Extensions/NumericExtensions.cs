...
public static class NumericExtensions
{
    ...
    public static string Percent(this GradedMetric<double> value, int digits = 2, bool? isZeroToOne = null, bool? plusIfPositive = null) => value.Value.Percent(digits, isZeroToOne, plusIfPositive);
    public static string Percent(this double value, int digits = 2, bool? isZeroToOne = null, bool? plusIfPositive = null) => $"{(plusIfPositive is true ? value.PlusIfPositive() : string.Empty)}{(value * (isZeroToOne is true ? 100 : 1)).Round(digits).ToString($"F{digits}")} {Constants.STRING_PERCENT}";
